...
class course_module_into_sharing_cart extends external_api {
    ...
    public static function execute(int $course_module_id): object {
        global $USER;
        ...
        $item_id = $base_factory->item()->repository()->insert_activity($params['course_module_id'], $USER->id, null, 0);
        $item = $base_factory->item()->repository()->get_by_id($item_id);

        $base_factory->backup()->handler()->backup_course_module($course_module_id, $item);
